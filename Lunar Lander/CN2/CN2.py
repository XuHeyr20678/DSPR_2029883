...
if __name__ == "__main__":
    ...
    #  Lunar DATASET
     ...
     rules = cn2.fit('train_MDLP.csv')
     train_end = time.time()
     print('Training time: ', train_end-train_start, ' s')
     print('Rules:')
     cn2.print_rules(rules)
    
     with open('../Data/output/lunar_rules_MDLP', 'wb') as f:
         pickle.dump(rules, f)
    ...
    # #     rules = pickle.load(f)
     ...
     rules_performance, accuracy = cn2.predict('test_MDLP.csv', rules)
    ...
    #
     ...
     with pd.option_context('display.max_rows', None, 'display.max_columns', None):
         print(table)
     table.to_csv('../Data/output/performance_MDLP.csv')
